	...
	function resume_x_about_setup()
	{
		...
		$config = array(
			...
			'quick_links' => array(
				...
				'update_url' => array(
					'text'   => esc_html__('PortfoliX PRO Video', 'resume-x'),
			...
			'recommended_actions'        => array(
				...
				'content'            => array(
					'magical-blocks' => array(
						'title'       => __('Magical Posts Display', 'resume-x'),
						'description' => __('Now you can add or update your site elements very easily by Magical Products Display. Supercharge your Elementor block with highly customizable Magical Blocks For WooCommerce.', 'resume-x'),
...
{
	global $pagenow;
	if (get_option('resumex1') || $pagenow == 'themes.php') {
		return;
	}
...
function resume_x_new_optins_texts_init()
{
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		//delete_option('resumex1');
		update_option('resumex1', 1);
	}
}
